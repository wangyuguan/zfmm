...
C$OMP END PARALLEL DO       
     ...
     1     iparent,nchild,ichild,iper,isep,mnbors,nnbors,nbors)

        if(nlevels.ge.2) then
          call pts_tree_fix_lr(centers,nlevels,nboxes,boxsize,nbmax,
     1         nlmax,iper,isep,laddr,ilevel,iparent,nchild,ichild,
     2         mnbors,nnbors,nbors)
        endif
      endif

c      ltree = 17*nboxes + 2*(nlevels+1) 
      ltree = (8+mnbors)*nboxes + 2*(nlevels+1) 
...
c
     ...
     1  nlmin,nlmax,ifunif,iper,isep,nlevels,nboxes,ltree,itree,iptr,
     2  centers,boxsize)
c
c
c----------------------------------------
...
c        do j=1,9
        do j=1,mnbors
c          itree(iptr(7)+9*(i-1)+j-1) = -1
          itree(iptr(7)+mnbors*(i-1)+j-1) = -1
        enddo
      enddo

      call computecoll(nlevels,nboxes0,itree(iptr(1)),boxsize,centers,
     1        itree(iptr(3)),itree(iptr(4)),itree(iptr(5)),iper,isep,
     2        mnbors,itree(iptr(6)),itree(iptr(7)))

      if(nlevels.ge.2) then
         call pts_tree_fix_lr(centers,nlevels,
     1         nboxes0,boxsize,nboxes,nlevels,iper,isep,itree(iptr(1)),
     2         itree(iptr(2)),itree(iptr(3)),itree(iptr(4)),
     3         itree(iptr(5)),mnbors,itree(iptr(6)),itree(iptr(7)))
...
c-------------------------------------------------------------      
     ...
     1       boxsize,nbmax,nlmax,iper,isep,laddr,ilevel,iparent,
     1       nchild,ichild,mnbors,nnbors,nbors)
c
c
...
C$OMP END PARALLEL DO      
     ...
     2                   ichild,iper,isep,mnbors,nnbors,nbors)
...
c         call updateflags(ilev,nboxes,nlevels,laddr,nchild,ichild,
c     1                    nnbors,nbors,centers,boxsize,iflag)
         call updateflags(ilev,nboxes,nlevels,laddr,nchild,ichild,
     1      isep,mnbors,nnbors,nbors,centers,boxsize,iflag)
c         call updateflags(ilev,nboxes,nlevels,laddrtail,nchild,ichild,
c     1                    nnbors,nbors,centers,boxsize,iflag)
...
c               Check if kbox is a nearest neighbor or in list 2
     ...
     1               isep*1.05*boxsize(ilev+1)).and.
     2                   (abs(centers(2,kbox)-centers(2,ibox)).le.
     3               isep*1.05*boxsize(ilev+1))) then
...
c
     ...
     1   iptr,centers,ixy,ixyse)
      implicit real *8 (a-h,o-z)
      integer ltree
      integer n,nboxes,nlevels,itree(ltree),iptr(8)
